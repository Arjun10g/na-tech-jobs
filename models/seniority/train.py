"""Train the seniority classifier.
...
embeddings + multinomial logistic regression. Runs in ~30 s on CPU.
    ...
    uv run python -m models.seniority.train
    uv run python -m models.seniority.train --encoder BAAI/bge-small-en-v1.5
"""
...
logger = logging.getLogger("models.seniority.train")

SPEC = ClassifierSpec(
    name="seniority",
    label_column="seniority_extracted",
    # Drop the regex's "mid" fallback (assigned to titles with no explicit
    # seniority signal) — those rows are unreliable supervision.
    drop_labels=("mid",),
)
